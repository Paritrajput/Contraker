...
export async function GET() {
    try {
        ...
        const tendersWithBlockchainData = await Promise.all(tenders.map(async (tender) => {
            try {
                console.log(Number(tender.blockchainTenderId))
                const tenderData = await contract.getTender(Number(tender.blockchainTenderId)); 
                ...
                return {
                    ...
                    blockchainData: {
                        ...
                        bidOpeningDate: new Date(Number(tenderData[6]) * 1000).toLocaleDateString(), // Convert timestamp
                        bidClosingDate: new Date(Number(tenderData[7]) * 1000).toLocaleDateString(),
